...
@section('heading', __('Quiz activity logs'))

@section('content')
    @include('admin.monitoring._tabs')

    <form method="GET" action="{{ route('admin.monitoring.quizzes') }}" class="mb-6 grid gap-3 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 md:grid-cols-5">
        <div>
        ...
        <div class="md:col-span-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('admin.monitoring.quizzes') }}" class="text-sm text-zinc-400 hover:text-white">{{ __('Reset') }}</a>
        </div>
    </form>
                ...
                @foreach($logs as $l)
                    <tr class="hover:bg-zinc-800/30">
                        ...
                        <td class="px-5 py-3 text-zinc-400">
                            ...
                            @if($l->lesson)<div class="text-xs text-zinc-500">{{ __('Lesson:') }} {{ $l->lesson->title }}</div>@endif
                            <div class="text-xs text-zinc-500">{{ __('Quiz:') }} {{ $l->quiz->title }}</div>
                        </td>
                        <td class="px-5 py-3 text-xs text-zinc-500">
                            @if($l->score_percent !== null)
                                <div>{{ __('Score:') }} <span class="text-zinc-300">{{ $l->score_percent }}%</span></div>
                            @endif
                            @if($l->passed !== null)
                                <div>{{ __('Passed:') }} <span class="text-zinc-300">{{ $l->passed ? 'yes' : 'no' }}</span></div>
                            @endif
                            @if($l->duration_seconds !== null)
                                <div>{{ __('Duration:') }} <span class="text-zinc-300">{{ $l->duration_seconds }}s</span></div>
                            @endif
                            @if(is_array($l->meta) && !empty($l->meta))
                                <div class="mt-1 text-[10px] text-zinc-600">{{ __('meta…') }}</div>
                            @endif
                        </td>
                    </tr>
                @endforeach
