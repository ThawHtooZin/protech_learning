...
@section('heading', __('Lesson activity logs'))

@section('content')
    @include('admin.monitoring._tabs')

    <form method="GET" action="{{ route('admin.monitoring.lessons') }}" class="mb-6 grid gap-3 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 md:grid-cols-5">
        <div>
        ...
        <table class="w-full text-left text-sm">
            <thead class="border-b border-zinc-800 text-xs uppercase tracking-wider text-zinc-500">
                <tr>
                    ...
                    <th class="px-5 py-3">{{ __('Lesson') }}</th>
